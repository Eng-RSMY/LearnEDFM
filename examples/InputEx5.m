...
Nf      = [200 200];  	                % number of cells in x and y direction

%% SIMULATION PARAMETER FOR TRANSPORT --------------------------------------------------------------%
global dt
timeSim  = 10000;%7*86400;                         % total simulation time [s]
dt       = 3600;                           % time step length [s]
...
%% PERMEABILITY ------------------------------------------------------------------------------------%
K       = ones(Nf(1),Nf(2))*1e-11; 	    % permeability field [m2]
K_f     = ones(Nf_f,1)*1e-6; 	        % fracture permeability field [m2]

%% Porosity ----------------------------------------------------------------------------------------%
phi     = ones(Nf(1),Nf(2))*0.1;	    % porosity field
phi_f   = ones(Nf_f,1)*0.1;             % fracture porosity field
...
lambda_s = 2.0;                           % Thermal conductivity of the rock [W/(m*K)]
cp_l = 1100;                               % Specific heat capacity of the fluid [J/(kg*K)]
cp_s = 250;                               % Specific heat capacity of the rock [J/(kg*K)]
