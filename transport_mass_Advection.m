function [Up,rhs] = transport_mass_Advection(vx,vy,vf,Vfm,Vmf,Vff,Q, QC ,Nf,Nf_f,Nf_i,c)
%transportAdvection
%  ---------------------------------------------------------------------
...
if innerIter < 2
 ...
 deltax(2:n(1),:) = c(2:n(1),:) - c(1:n(1)-1,:);
 deltay(:,2:n(2)) = c(:,2:n(2)) - c(:,1:n(2)-1);
...
if (scheme == 1)  % This is the QUICK scheme
    ...
    upw(1:n(1),:) = -ix(1:n(1),:)       .*vx(1:n(1),:)   .* (1-limx(1:n(1),:));
    ups(:,1:n(2)) = -iy(:,1:n(2))       .*vy(:,1:n(2))   .* (1-limy(:,1:n(2)));
    upe(1:n(1),:) =  (1-ix(2:n(1)+1,:)) .*vx(2:n(1)+1,:) .* (1-limx(2:n(1)+1,:));
    upn(:,1:n(2)) =  (1-iy(:,2:n(2)+1)) .*vy(:,2:n(2)+1) .* (1-limy(:,2:n(2)+1));

    upd(:,:)      = ix(2:n(1)+1,:)   .*vx(2:n(1)+1,:) .*(1-limx(2:n(1)+1,:))...
                   +iy(:,2:n(2)+1)   .*vy(:,2:n(2)+1) .*(1-limy(:,2:n(2)+1))...
                   -(1-ix(1:n(1),:)) .*vx(1:n(1),:)   .*(1-limx(1:n(1),:))...
                   -(1-iy(:,1:n(2))) .*vy(:,1:n(2))   .*(1-limy(:,1:n(2)));
    ...
    upwl(1:n(1),:) = -ix(1:n(1),:)      .*vx(1:n(1),:)   .* limx(1:n(1),:);
    upsd(:,1:n(2)) = -iy(:,1:n(2))      .*vy(:,1:n(2))   .* limy(:,1:n(2));
    upel(1:n(1),:) =  ix(2:n(1)+1,:)    .*vx(2:n(1)+1,:) .* limx(2:n(1)+1,:);                                                                                 
    upnd(:,1:n(2)) =  iy(:,2:n(2)+1)    .*vy(:,2:n(2)+1) .* limy(:,2:n(2)+1);

    upwr(1:n(1),:) = -(1-ix(1:n(1),:))  .*vx(1:n(1),:)   .* limx(1:n(1),:);
    upsu(:,1:n(2)) = -(1-iy(:,1:n(2)))  .*vy(:,1:n(2))   .* limy(:,1:n(2));
    uper(1:n(1),:) =  (1-ix(2:n(1)+1,:)).*vx(2:n(1)+1,:) .* limx(2:n(1)+1,:);                                                                                 
    upnu(:,1:n(2)) =  (1-iy(:,2:n(2)+1)).*vy(:,2:n(2)+1) .* limy(:,2:n(2)+1);
...
rhs(iD1,1) = rhs(iD1,1) - t1.*FixC(i1,1);
rhs(iD2,1) = rhs(iD2,1) - t2.*FixC(i2,1);
rhs(iD3,1) = rhs(iD3,1) - t3.*FixC(i3,1);
rhs(iD4,1) = rhs(iD4,1) - t4.*FixC(i4,1);
