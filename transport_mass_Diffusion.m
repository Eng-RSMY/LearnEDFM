function [Di,ri] = transport_mass_Diffusion(Nf,Nf_f,dx,phix,phiy)
...
global DifC ibcDC FixC

Dx = dx(2).*phix.*DifC./dx(1);
Dy = dx(1).*phiy.*DifC./dx(2);
...
t1(1:Nf(2),1)  = Dx(1,1:Nf(2))'.*ibcDC(i1);   
t2(1:Nf(2),1)  = Dx(Nf(1)+1,1:Nf(2))'.*ibcDC(i2);
t3(1:Nf(1),1)  = Dy(1:Nf(1),1) .*ibcDC(i3);   
t4(1:Nf(1),1)  = Dy(1:Nf(1),Nf(2)+1) .*ibcDC(i4);   

iD  = [ic1';ic2';ic3';ic4'];
tD  = [t1;t2;t3;t4];
Di  = Di + sparse(iD,iD,tD,prod(Nf),prod(Nf));                     

ri(ic1,1) = ri(ic1,1) + t1.*FixC(i1,1);
ri(ic2,1) = ri(ic2,1) + t2.*FixC(i2,1);
ri(ic3,1) = ri(ic3,1) + t3.*FixC(i3,1);
ri(ic4,1) = ri(ic4,1) + t4.*FixC(i4,1);
